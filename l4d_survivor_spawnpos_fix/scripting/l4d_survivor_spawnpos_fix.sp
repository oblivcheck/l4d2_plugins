...
stock void DistanceInfo(int client)
{
	...
	PrintToServer("%N distance = %f. Stuck? %b", client, dist, bStuck);
}
...
stock bool SurvivorEntityInfo()
{
	...
	{
		PrintToServer("Failed to find spawn position!");
		return false;
	}
	else {
		PrintToServer("survivor warp position: %f %f %f", pos[0], pos[1], pos[2]);
	}
	return true;
}
...
Action CmdSpawnWarp(int client, int args)
{
	...
	for(int i=1;i<=MaxClients;i++)
	{
		if(IsClientInGame(i) )
		{
			TeleportToSafeRoom(i, TELEPORT_METHOD_WARP);
		}
	}
...
Action CmdSpawnTel(int client, int args)
{
	...
	for(int i=1;i<=MaxClients;i++)
	{
		if(IsClientInGame(i) )
		{
			TeleportToSafeRoom(i, TELEPORT_METHOD_TELEPORT);
		}
	}
/*
...
// 2023-12-03 确保c6的剧情机器人不会被传送，RC服务器只需要这个插件的cmd
//	InitHook();
}
...
void Fix_InfoPlayerStartEntity()
{
	...
	{
		#if DEBUG
			PrintToServer("Found %i info_survivor_position entities.", count);
		#endif
		...
		for( int i = 4; i < count; i++ )
		{
			...
			if( dist > DISTANCE_TO_WARP_MAX )
			{
				...
				#if DEBUG
					PrintToServer("info_survivor_position %i teleported to %i.", entity, iEntityFirst);
				#endif
			}
		}
	}
